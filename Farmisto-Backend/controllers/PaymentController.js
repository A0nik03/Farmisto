...
const createPayment = async (req, res) => {
  try {
    ...
    doc.fontSize(14).text(`Buyer Name: ${req.user.name}`, { align: "left" }).moveDown(0.5);
    ...
    doc.moveTo(30, tableTop + 15).lineTo(600, tableTop + 15).stroke();
    ...
    updatedCartItems.forEach((item) => {
      doc.fillColor('#000000')
        ...
        .text(`Rs ${item.itemPrice.toFixed(2)}`, 180, currentY, { width: 100, align: 'right' })
        .text(`${item.discount}%`, 280, currentY, { width: 80, align: 'right' })
        .text(item.quantity.toString(), 380, currentY, { width: 80, align: 'right' })
        .text(`Rs ${item.discountedPrice}`, 480, currentY, { width: 100, align: 'right' })
        .text(`Rs ${item.totalItemCost}`, 600, currentY, { width: 100, align: 'right' });
    ...
    farmers.forEach((farmer, index) => {
      doc.fillColor('#000000')
        .fontSize(10).font('Helvetica')
        .text(`${index + 1}. ${farmer.name} (${farmer.email})`, 40, currentY + 50 + (index * 20));
    });
...
const GetPayments = async (req, res) => {
  ...
  try {
    const payments = await Payment.find({ "farmers.id": req.user.id });
